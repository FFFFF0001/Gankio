...
public class ImageLoader {
    ...
    public static ImageLoader getInstance(){
        if (instance == null){
            synchronized (ImageLoader.class){
                if (instance == null){
                    instance = new ImageLoader(GankApp.getContext());
                }
            }
        }
        return instance;
    ...
    private void initDiskLruCache() {
        ...
        if (getUsableSpace(diskCacheDir) > DISK_CACHE_SIZE) {
            try {
                mDiskLruCache = com.jakewharton.disklrucache.DiskLruCache.open(diskCacheDir, 1, 1, DISK_CACHE_SIZE);
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
    ...
    private Bitmap loadBitmapFromHttp(String urlString, int reqWidth, int reqHeight) throws IOException {
        ...
        com.jakewharton.disklrucache.DiskLruCache.Editor editor = mDiskLruCache.edit(key);
        if (editor != null) {
